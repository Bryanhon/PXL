...
using namespace std;
int MyIdea( struct Game * game, struct Player * player, unsigned int totalBet )
{
	PokerRank myRank = getMyHandRank( player->hand );
	if(
			(myRank.category == HIGH_CARD
			 || ( myRank.category >= ONE_PAIR && myRank.category <= TWO_PAIR && myRank.rank < 8 ) )
		 && game->amountHands > 3 && totalBet > player->bet * 2 )
	{
		return( -1 );
	}
	if(player->hand->cards[0]->rank > 5 || player->hand->cards[1]->rank > 5)
	{
		return(0);
	}
	return(-1);
...
int willYouRaise( struct Game * game, struct Player * player, unsigned int totalBet )
{
    ...
    {
	case 6:
		return( MyIdea( game, player, totalBet ) );
		break;
	case 0:
