...
byte compareCards( struct Card * lhs, struct Card * rhs )
{
...
byte comparePokerRanks( struct PokerRank lhs, struct PokerRank rhs )
{
	if( lhs.category < rhs.category || ( lhs.category == rhs.category && ( lhs.rank < rhs.rank || ( lhs.rank == rhs.rank && handValue( lhs ) < handValue( rhs ) ) ) ) )
	{
		return( -1 );
	}
	else if( lhs.category > rhs.category || ( lhs.category == rhs.category && ( lhs.rank > rhs.rank || ( lhs.rank == rhs.rank && handValue( lhs ) > handValue( rhs ) ) ) ) )
	{
		return( 1 );
	}
	else
...
PokerRank getMyHandRank( struct Hand * hand )
{
	...
	byte comboSize = COMBO_SIZE;
	for( byte i = 0 ; i < 2 ; i++ ) //fill
	{
		combo[i] = hand->cards[i];
	}
	for( byte i = 0 ; i < 5 ; i++ ) //fill
	{
		if( table[i] == nullptr )
		{
			comboSize = i+2;
	...
	for( byte i = 0 ; i < comboSize ; i++ ) //sort
	{
		if( combo[i] != nullptr )
		{
			...
			{
				if( combo[j] != nullptr && compareCards( combo[j], combo[i] ) < 0 )
				{
...
void bettingInGame( struct Game * game, byte isPreFlop )
{
	...
	while( ( game->players[i]->bet < game->highestBet ) || ( game->amountHands > 1 && minimumPlayerCheck > 0 ) )
	{
		...
		if( isHandFilled( game->players[i]->hand ) > 0 && game->players[i]->chips != game->players[i]->bet ) //playing and not all in yet
		{
			...
			if( raisedBet < 0 && game->players[i]->bet < game->highestBet )
			{
...
void showdownInGame( struct Game * game )
{
	...
	else
	{
		for( byte i = 0 ; i < showedCardsIndex ; i++ ) //sort
		{
			...
			{
				byte comp = comparePokerRanks( showedCards[i]->rank, showedCards[j]->rank );
				if( comp == 1 || ( comp == 0 && showedCards[i]->player->bet < showedCards[j]->player->bet ) )
				{
		...
		while( game->highestBet > 0 && showedCardsIndex > 0 )
		{
			...
			for( i = 2 ; i <= showedCardsIndex && ( comparePokerRanks( showedCards[showedCardsIndex-i]->rank, showedCards[showedCardsIndex-1]->rank ) == 0 ) ; i++ )
			...
			if( i > 1 )
			{
				...
				if( rest > 0 )
				{
					...
					for( i = 1 ; i <= showedCardsIndex && ( comparePokerRanks( showedCards[showedCardsIndex-i]->rank, showedCards[showedCardsIndex-1]->rank ) == 0 ) ; i++ )
					{
